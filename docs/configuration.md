...
| `SKIP_FORCED_PUSHES` | no | no | `false` | At the default, a `forced: true` push posts one rollup message labelled as a force push, never per-commit tables, because a rebase gives every commit a new SHA and SHA dedup cannot stop the double-announce. The rollup links to the new head commit and its Files row reads `N/A`: after a history rewrite the old and new tips share no commits, so a comparison or file count would cover the whole repository. Set it to `true` to post nothing at all, logged `push_skipped { reason: "forced_push" }` |
